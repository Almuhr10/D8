...
$(document).on('ready', function () {
  ...
  var lastEl = "";
  var finalEl= "";
	$(document).on('click', '.english-menu', function(e){
		basic_check = url.split("/");
    lastEl= basic_check[basic_check.length-1];
    if(lastEl.trim() == "" || url.indexOf("index") >= 0 )
    {
      if(url.indexOf("index") >= 0)
      {
        new_urlEN = "";
        new_urlEN=url.replace("index","index-en");
      }
      else
      {
        new_urlEN = "";
        new_urlEN=url+"index-en";
      }
    }
    else
    {
      new_urlEN = "";
     // console.log(lastEl);
      finalEl = lastEl.replace('.', '')+"-en";
      //console.log(lastEl);
      new_urlEN=url.replace(lastEl,'')+finalEl;
      //console.log(new_urlEN);
      
    }
		
    window.location.href = new_urlEN;
	});
	
	$(document).on('click', '.arabic-menu', function(e){
		
		new_urlAR = "";
    new_urlAR=url.replace("-en", "");

		window.location.href = new_urlAR;
	});
	//END PAGE REDIRECT AR-EN
